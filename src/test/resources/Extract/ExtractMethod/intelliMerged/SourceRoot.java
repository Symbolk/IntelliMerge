...
public class SourceRoot {
  ...
  /**
   * Parses all .java files in a package recursively, caches them, and returns all files ever parsed
   * with this source root.
   */
  public Map<Path, ParseResult<CompilationUnit>> tryToParse(String startPackage)
      throws IOException {
  ...
  public ParseResult<CompilationUnit> tryToParse(String packag, String filename)
      throws IOException {
  ...
  /**
   * Parses a package recursively with a callback. The advantage: it doesn't keep all files and
   * AST's in memory. The disadvantage: you have to do your processing directly.
   */
  public void parse(String startPackage, JavaParser javaParser, Callback callback)
      throws IOException {
    Log.info("Parsing package \"%s\"", startPackage);
    final Path path = packageAbsolutePath(root, startPackage);
    Files.walkFileTree(
          ...
          public FileVisitResult visitFile(Path absolutePath, BasicFileAttributes attrs)
            ...
            if (!attrs.isDirectory() && absolutePath.toString().endsWith(".java")) {
              ...
              final ParseResult<CompilationUnit> result =
                  javaParser.parse(COMPILATION_UNIT, provider(absolutePath));
